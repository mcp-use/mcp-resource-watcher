...
var TUNNEL_URL_REGEX = /https?:\/\/([a-z0-9-]+\.[a-z0-9.-]+)/i;
...
function readSubdomainFromManifest() {
  ...
  try {
    ...
    return manifest?.tunnel?.subdomain || void 0;
  } catch {
    return void 0;
  }
}
async function startTunnel(subdomain) {
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.stderr?.on("data", (data) => {
      const text = data.toString();
      if (!text.includes("INFO") && !text.includes("bore_cli") && !text.includes("Shutting down")) {
...
`));
  ...
  app2.get("/inspector/api/tunnel/status", (c) => {
    const status = getTunnelStatus();
    return c.json(status);
  });
  app2.post("/inspector/api/tunnel/start", async (c) => {
    try {
  ...
  app2.delete("/inspector/api/tunnel/stop", (c) => {
    const stopped = stopTunnel();
    return c.json({ stopped });
  });
  app2.get("/inspector/api/dev/info", (c) => {
    const mcpUrl2 = process.env.MCP_URL ?? null;
    ...
    if (!tunnelUrl && mcpUrl2) {
      try {
        const u = new URL(mcpUrl2);
  ...
  app2.post("/inspector/api/dev/start-tunnel", (c) => {
    const restart = globalThis.__mcpUseDevRestart;
    if (!restart) {
      return c.json(
  ...
  app2.post("/inspector/api/dev/stop-tunnel", (c) => {
    const restart = globalThis.__mcpUseDevRestart;
    if (!restart) {
      return c.json(
        { error: "Dev restart not available (not running via mcp-use dev)" },
  ...
  app2.post("/inspector/api/dev/restart-with-tunnel", (c) => {
    const restart = globalThis.__mcpUseDevRestart;
    if (!restart) return c.json({ error: "Not available" }, 500);
    setTimeout(() => restart(true), 200);
    return c.json({ ok: true, restarting: true });
  });
  app2.post("/inspector/api/dev/restart-without-tunnel", (c) => {
    const restart = globalThis.__mcpUseDevRestart;
    if (!restart) return c.json({ error: "Not available" }, 500);
    setTimeout(() => restart(false), 200);
    return c.json({ ok: true, restarting: true });
...
function registerStaticRoutes(app2, clientDistPath, runtimeConfig) {
  const effectiveConfig = {
    ...runtimeConfig,
    proxyUrl: runtimeConfig?.proxyUrl !== void 0 ? runtimeConfig.proxyUrl : "/inspector/api/proxy"
  };
  if (USE_CDN) {
    const serveShell = (c) => c.html(generateCdnShellHtml(effectiveConfig));
    app2.get("/", (c) => {
      const url = new URL(c.req.url);
  ...
  app2.get("/inspector/assets/*", (c) => {
    ...
    const fullPath = join3(distPath, path);
