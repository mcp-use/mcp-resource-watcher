...
import {
  ...
  isJsonRpcRequest,
  isJsonRpcResponse
} from "../../chunk-TPPVC4QO.js";
import "../../chunk-KUEVOU4M.js";
import {
  Telemetry,
  VERSION,
  getPackageVersion
} from "../../chunk-EXKP7A6D.js";
...
async function startServer(app, port, host, options) {
  if (isDeno) {
    ...
    return {
      close: /* @__PURE__ */ __name(() => new Promise((resolve, reject) => {
        ...
        if (isWidgetFile) {
          ...
          if (widget2) {
            try {
              ...
              import("../../tool-registry-generator-CQTITKOX.js").then(
...
function registerPrompt(promptDefinition, callback) {
  ...
  const wrappedCallback = /* @__PURE__ */ __name(async (params, extra) => {
    ...
    const { findSessionContext: findSessionContext2, createClientCapabilityChecker: createClientCapabilityChecker2 } = await import("../../tool-execution-helpers-F3HGXPSA.js");
...
function registerResourceTemplate(resourceTemplateDefinition, callback) {
  ...
  this.server.registerResource(
    ...
    async (uri) => {
      ...
      const { getRequestContext: getRequestContext2, runWithContext: runWithContext2 } = await import("../../context-storage-KMOAHEL7.js");
      const { findSessionContext: findSessionContext2 } = await import("../../tool-execution-helpers-F3HGXPSA.js");
      const initialRequestContext = getRequestContext2();
      const sessions = this.sessions || /* @__PURE__ */ new Map();
...
async function requestLogger(c, next) {
  ...
  if (mcpMethod) {
    if (isInitialize) {
      ...
      parts.push(chalk2.bold(`[resources/read: ${uri}]`));
    } else if (mcpMethod === "prompts/get") {
      const promptName = requestBody?.params?.name ?? "?";
      parts.push(chalk2.bold(`[prompts/get: ${promptName}]`));
    } else {
      parts.push(chalk2.bold(`[${mcpMethod}]`));
    }
  }
  if (isInitialize && responseSid) {
    parts.push(chalk2.gray(`\u2192 session=${shortSessionId(responseSid)}`));
  }
  let outcomePart;
  const errMsg = await extractResponseError(c.res);
  if (errMsg) {
    outcomePart = chalk2.red(`ERROR ${errMsg}`);
  } else if (mcpMethod) {
    outcomePart = statusCode >= 400 ? chalk2.red(`ERROR (HTTP ${statusCode})`) : chalk2.green("OK");
  } else {
    outcomePart = statusCode >= 500 ? chalk2.magenta(String(statusCode)) : statusCode >= 400 ? chalk2.red(String(statusCode)) : statusCode >= 300 ? chalk2.yellow(String(statusCode)) : chalk2.green(String(statusCode));
  }
  parts.push(outcomePart);
  parts.push(chalk2.gray(`(${durationMs}ms)`));
  console.log(parts.join(" "));
  if (level !== "trace") return;
  console.log("\n" + chalk2.cyan("=".repeat(80)));
  console.log(chalk2.bold.cyan("[TRACE] Request Details"));
  console.log(chalk2.cyan("-".repeat(80)));
  if (Object.keys(requestHeaders).length > 0) {
    console.log(chalk2.yellow("Request Headers:"));
    console.log(formatForLogging(requestHeaders));
  }
  if (requestBody !== null) {
    console.log(chalk2.yellow("Request Body:"));
  ...
  if (Object.keys(responseHeaders).length > 0) {
    console.log(chalk2.yellow("Response Headers:"));
...
... (truncated, ${responseBody.length - maxLength} more characters)`
          ...
          console.log(chalk2.yellow("Response Body:") + " (empty)");
        }
      } catch {
        console.log(chalk2.yellow("Response Body:") + " (unable to clone/read)");
      }
    } else {
      console.log(chalk2.yellow("Response Body:") + " (no body)");
    }
  } catch {
    console.log(chalk2.yellow("Response Body:") + " (unable to read)");
  }
  console.log(chalk2.cyan("=".repeat(80)) + "\n");
}
...
var MCPServerClass = class {
  ...
  async proxy(config, options) {
    ...
    const { mountSession } = await import("../../proxy-client-GDPUOT5O.js");
  ...
  async listen(port) {
    ...
    if (process.env.NODE_ENV !== "production") {
      try {
        const { generateToolRegistryTypes: generateToolRegistryTypes2 } = await import("../../tool-registry-generator-CQTITKOX.js");
...
import { jwtVerify as jwtVerify3, createRemoteJWKSet as createRemoteJWKSet3, decodeJwt as decodeJwt2 } from "jose";
var ClerkOAuthProvider = class {
  ...
  async verifyToken(token) {
    ...
    try {
      ...
      const result = await jwtVerify3(token, this.getJWKS(), verifyOptions);
      return result;
    } catch (error2) {
...
import { jwtVerify as jwtVerify4, createRemoteJWKSet as createRemoteJWKSet4 } from "jose";
...
import { jwtVerify as jwtVerify5, createRemoteJWKSet as createRemoteJWKSet5, decodeJwt as decodeJwt3 } from "jose";
var WorkOSOAuthProvider = class {
  ...
  getJWKS() {
    if (!this.jwks) {
      this.jwks = createRemoteJWKSet5(new URL(`${this.issuer}/oauth2/jwks`));
    }
    return this.jwks;
  }
  async verifyToken(token) {
    ...
    try {
      const result = await jwtVerify5(token, this.getJWKS(), {
        issuer: this.issuer
      });
      return result;
    } catch (error2) {
...
import { jwtVerify as jwtVerify6, createRemoteJWKSet as createRemoteJWKSet6, decodeJwt as decodeJwt4 } from "jose";
var BetterAuthOAuthProvider = class {
  ...
  getJWKS() {
    if (!this.jwks) {
      this.jwks = createRemoteJWKSet6(new URL(this.jwksUrl));
    }
    return this.jwks;
  }
  async verifyToken(token) {
    if (this.config.verifyJwt === false) {
      ...
      const payload = decodeJwt4(token);
      return { payload };
    }
    try {
      const result = await jwtVerify6(token, this.getJWKS(), {
        issuer: this.issuer
      });
      return result;
    } catch (error2) {
...
import { jwtVerify as jwtVerify7, createRemoteJWKSet as createRemoteJWKSet7 } from "jose";
function jwksVerifier(config) {
  ...
  const jwks = createRemoteJWKSet7(new URL(config.jwksUrl));
  return async (token) => {
    try {
      const result = await jwtVerify7(token, jwks, {
