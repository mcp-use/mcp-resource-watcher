    ...
    JSONSchemaToZod = class {
      ...
      static parseObject(schema) {
        ...
        if (!schema.properties && schema.additionalProperties && typeof schema.additionalProperties === "object") {
          ...
          return import_zod.z.object({}).catchall(valueSchema);
        }
        const shape = {};
...
__export(index_exports, {
  ...
  reject: () => reject,
  setBrowserTelemetrySource: () => setTelemetrySource2,
...
var RemoteAgent = class {
  ...
  async createChatSession() {
    ...
    logger.debug(`\u{1F4DD} Creating chat session for agent ${this.agentId}`);
    try {
  ...
  async run(queryOrOptions, maxSteps, manageConnector, externalHistory, outputSchema) {
    ...
    try {
      logger.debug(`\u{1F310} Executing query on remote agent ${this.agentId}`);
      if (this.chatId === null) {
      ...
      if (schema) {
        ...
        logger.debug(`\u{1F527} Using structured output with schema`);
      }
      const headers = {
        "Content-Type": "application/json",
        "x-api-key": this.apiKey
      ...
      logger.debug(`\u{1F680} Executing agent in chat ${chatId}`);
      ...
      logger.debug(`\u{1F527} Response: ${JSON.stringify(result)}`);
...
Raw error: ${result}`
  ...
  async close() {
    logger.debug("\u{1F50C} Remote agent client closed");
  }
};
...
function parseLLMString(llmString) {
  ...
  if (parts.length !== 2) {
    throw new Error(
      `Invalid LLM string format. Expected 'provider/model', got '${llmString}'. Examples: 'openai/gpt-4', 'anthropic/claude-sonnet-4-6', 'google/gemini-pro', 'groq/llama-3.1-70b-versatile'`
    );
  }
...
var MCPAgent = class {
  ...
  async initialize() {
    ...
    logger.debug("\u{1F680} Initializing MCP agent and connecting to services...");
    if (this.isSimplifiedMode) {
      logger.debug(
    ...
    if (handlerNames.length > 0) {
      logger.debug(`\u{1F4CA} Observability enabled with: ${handlerNames.join(", ")}`);
    }
    if (this.useServerManager && this.serverManager) {
      ...
      if (this.client) {
        ...
        if (nonCodeModeSessions.length === 0) {
          logger.debug("\u{1F504} No active sessions found, creating new ones...");
        ...
        if (this.client.codeMode) {
          ...
          if (codeModeSession) {
            ...
            logger.debug(`\u{1F6E0}\uFE0F Created ${this._tools.length} code mode tools`);
          } else {
            throw new Error(
      ...
      logger.debug(
        `\u{1F9F0} Found ${this._tools.length} tools across all connectors`
      );
      await this.createSystemMessageFromTools(this._tools);
    }
  ...
  createAgent() {
    ...
    logger.debug(`\u{1F9E0} Agent ready with tools: ${toolNames.join(", ")}`);
  ...
  async *stream(queryOrOptions, maxSteps, manageConnector = true, externalHistory, outputSchema, signal) {
    ...
    try {
      ...
      logger.debug(`\u{1F4AC} Received query: '${displayQuery}'`);
      logger.debug("\u{1F3C1} Starting agent execution");
      const maxRestarts = 3;
      let restartCount = 0;
            ...
            if (nodeOutput && typeof nodeOutput === "object" && "messages" in nodeOutput) {
              ...
              for (const message of messages) {
                ...
                if (this._isToolMessageLike(message)) {
                  ...
                  logger.debug(`\u{1F4C4} Tool result: ${observationStr}`);
                  if (this.useServerManager && this.serverManager) {
                    const currentTools = this.serverManager.tools;
      ...
      if (schema && finalOutput) {
        try {
          logger.debug("\u{1F527} Attempting structured output...");
      ...
      if (initializedHere && manage) {
        logger.debug("\u{1F9F9} Cleaning up resources after error");
        await this.close();
      }
      throw e;
    } finally {
      ...
      if (manage && !this.client && initializedHere) {
        logger.debug("\u{1F9F9} Closing agent after stream completion");
        await this.close();
      }
    }
  }
  ...
  async close() {
    ...
    logger.debug("\u{1F50C} Closing MCPAgent resources\u2026");
    ...
    try {
      ...
      if (this.client) {
        if (this.clientOwnedByAgent) {
          ...
          logger.debug("\u{1F504} Closing client and cleaning up resources");
          await this.client.close();
          this.sessions = {};
        }
      } else {
        for (const connector of this.connectors) {
          logger.debug("\u{1F504} Disconnecting connector");
      ...
      logger.debug("\u{1F44B} Agent closed successfully");
    }
  }
  ...
  async *streamEvents(queryOrOptions, maxSteps, manageConnector = true, externalHistory, outputSchema, signal) {
    ...
    try {
      ...
      logger.debug(`\u{1F4AC} Received query for streamEvents: '${display_query}'`);
      if (this.memoryEnabled) {
        logger.debug(`\u{1F504} Adding user message to history: ${display_query}`);
      ...
      if (schema && finalResponse) {
        logger.debug("\u{1F527} Attempting structured output conversion...");
        try {
      ...
      console.log("\n\n");
      logger.debug(`\u{1F389} StreamEvents complete - ${eventCount} events emitted`);
      success = true;
    } catch (e) {
      ...
      if (manage && !this.client && initializedHere) {
        logger.debug("\u{1F9F9} Closing agent after streamEvents completion");
        await this.close();
      }
    }
  }
  ...
  async _attemptStructuredOutput(rawResult, llm, outputSchema) {
    ...
    logger.debug(`\u{1F504} Raw result: ${JSON.stringify(rawResult, null, 2)}`);
    ...
    logger.debug(`\u{1F504} Schema description: ${schemaDescription}`);
    let textContent = "";
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      logger.debug(`\u{1F504} Structured output attempt ${attempt}/${maxRetries}`);
      let formatPrompt = `
...
__name(secureRandomString2, "secureRandomString");
...
function detectRuntimeEnvironment2() {
  try {
...
__name(detectRuntimeEnvironment2, "detectRuntimeEnvironment");
function getStorageCapability2(env) {
  if (env === "browser") {
    try {
...
var Telemetry2 = class _Telemetry {
  ...
  _source;
  constructor() {
    this._runtimeEnvironment = getRuntimeEnvironment2();
    this._storageCapability = getStorageCapability2(this._runtimeEnvironment);
  ...
  get userId() {
    ...
    try {
      switch (this._storageCapability) {
        ...
        default:
          try {
            this._currUserId = `session-${generateUUID2()}`;
          } catch (uuidError) {
            this._currUserId = `session-${Date.now()}-${secureRandomString2()}`;
          }
          break;
      }
    } catch (e) {
  ...
  _getUserIdFromLocalStorage() {
    try {
      ...
      if (!userId) {
        try {
          userId = generateUUID2();
        } catch (uuidError) {
          userId = `${Date.now()}-${secureRandomString2()}`;
        }
      ...
      let sessionId;
      try {
        sessionId = `session-${generateUUID2()}`;
      } catch (uuidError) {
        sessionId = `session-${Date.now()}-${secureRandomString2()}`;
      }
      return sessionId;
    }
  }
...
var Tel2 = Telemetry2;
function setTelemetrySource2(source) {
  Tel2.getInstance().setSource(source);
}
__name(setTelemetrySource2, "setTelemetrySource");
...
function trackBrowserClientInit(config) {
  const servers = Object.keys(config.mcpServers ?? {});
  Tel2.getInstance().trackMCPClientInit({
    codeMode: false,
    sandbox: false,
    allCallbacks: false,
    verify: false,
...
function useMcp(options) {
  ...
  const oauthClientSecret = oauthOptions?.clientSecret?.trim() || void 0;
  const oauthScope = oauthOptions?.scope?.trim() || void 0;
  const staticClientInfo = (0, import_react.useMemo)(
    () => oauthClientId ? {
  ...
  const autoReconnectConfig = (0, import_react.useMemo)(() => {
    ...
    return {
      ...
      healthCheckInterval: autoReconnect.healthCheckInterval ?? 1e4,
      healthCheckTimeout: autoReconnect.healthCheckTimeout ?? 3e4
    };
  }, [autoReconnect]);
  ...
  const failConnection = (0, import_react.useCallback)(
    (errorMessage, connectionError) => {
      ...
      connectingRef.current = false;
      if (url) {
        Tel2.getInstance().trackUseMcpConnection({
          url,
          transportType,
          success: false,
    ...
    const tryConnectWithTransport = /* @__PURE__ */ __name(async (transportTypeParam) => {
      ...
      try {
        ...
        const serverConfig = {
          ...
          ...reconnectionOptions ? { reconnectionOptions } : autoReconnect === false ? { reconnectionOptions: { maxRetries: 0 } } : {}
        };
        if (gatewayUrl) {
        ...
        if (autoReconnectConfig.enabled && autoReconnectConfig.healthCheckInterval !== false) {
        ...
        if (session.connector.serverCapabilities?.resourceTemplates) {
          const templatesResult = await session.connector.listResourceTemplates();
          if (!isMountedRef.current) {
            addLog(
              "debug",
  ...
  const callTool = (0, import_react.useCallback)(
    async (name, args, options2) => {
      ...
      try {
        ...
        Tel2.getInstance().trackUseMcpToolCall({
          toolName: name,
          success: true,
    ...
    if (currentState === "failed") {
      ...
      try {
        ...
        if (providedAuthProvider) {
          ...
          const baseUrl2 = parsedUrl2.origin + parsedUrl2.pathname.replace(/\/+$/, "");
          await (0, import_auth4.auth)(authProviderRef.current, {
            serverUrl: baseUrl2
          });
          connectRef.current?.();
  ...
  const readResource = (0, import_react.useCallback)(
    async (uri) => {
      ...
      try {
        ...
        Tel2.getInstance().trackUseMcpResourceRead({
          resourceUri: uri,
          success: true
        }).catch(() => {
        });
  ...
  const refreshResourceTemplates = (0, import_react.useCallback)(async () => {
    if (stateRef.current !== "ready" || !clientRef.current) {
  ...
  const complete = (0, import_react.useCallback)(
    async (params) => {
      ...
      const refType = params.ref.type === "ref/prompt" ? "prompt" : "resource template";
...
// src/react/model-context.tsx
var import_react2 = __toESM(require("react"), 1);
var MODEL_CONTEXT_KEY = "__model_context";
var nodes = /* @__PURE__ */ new Map();
function setNode(node) {
  nodes.set(node.id, node);
  scheduleFlush();
}
__name(setNode, "setNode");
...
__name(buildDescriptionString, "buildDescriptionString");
var flushHandler = null;
function registerModelContextFlush(handler) {
...
__name(registerModelContextFlush, "registerModelContextFlush");
function notifyHost(description) {
  flushHandler?.(description);
}
__name(notifyHost, "notifyHost");
var ParentIdContext = (0, import_react2.createContext)(null);
function ModelContext({ content, children }) {
  ...
  return /* @__PURE__ */ import_react2.default.createElement(ParentIdContext.Provider, { value: id }, children);
}
__name(ModelContext, "ModelContext");
var modelContext = {
  /**
...
var ErrorBoundary = class extends import_react3.default.Component {
  ...
  render() {
    if (this.state.hasError && this.state.error) {
      ...
      return /* @__PURE__ */ import_react3.default.createElement("div", { className: "p-4 border border-red-500 bg-red-50 text-red-900 rounded-md dark:bg-red-900/20 dark:text-red-100" }, /* @__PURE__ */ import_react3.default.createElement("h3", { className: "font-bold mb-2" }, "Widget Error"), /* @__PURE__ */ import_react3.default.createElement("pre", { className: "text-sm whitespace-pre-wrap" }, this.state.error.message));
    }
...
var Image = /* @__PURE__ */ __name(({ src, ...props }) => {
  ...
  return /* @__PURE__ */ import_react4.default.createElement("img", { src: finalSrc, ...props });
}, "Image");
...
function useWidget(defaultProps) {
  ...
  const [mcpAppsHostInfo, setMcpAppsHostInfo] = (0, import_react5.useState)(null);
  const [mcpAppsHostCapabilities, setMcpAppsHostCapabilities] = (0, import_react5.useState)(null);
  ...
  const localWidgetStateRef = (0, import_react5.useRef)(localWidgetState);
  ...
  (0, import_react5.useEffect)(() => {
    const deregister = registerModelContextFlush((description) => {
      ...
      if (currentProvider === "openai" && window.openai?.setWidgetState) {
        const prev = window.openai.widgetState ?? localWidgetStateRef.current ?? {};
  ...
  const sendFollowUpMessage = (0, import_react5.useCallback)(
    async (content) => {
      const contentArray = typeof content === "string" ? [{ type: "text", text: content }] : content;
      if (provider === "mcp-apps") {
        const bridge = getMcpAppsBridge();
      ...
      const prompt = typeof content === "string" ? content : contentArray.filter(
  ...
  const setState = (0, import_react5.useCallback)(
    async (state) => {
      if (provider === "mcp-apps") {
        ...
        const bridge = getMcpAppsBridge();
        const structuredContent = newState2;
        bridge.updateModelContext({
          structuredContent,
          content: [
            {
      ...
      const prevModelContext = (window.openai.widgetState ?? {})[MODEL_CONTEXT_KEY];
      setLocalWidgetState(newState);
      return window.openai.setWidgetState(
        prevModelContext !== void 0 ? { ...newState, [MODEL_CONTEXT_KEY]: prevModelContext } : newState
      );
    },
  ...
  return {
    ...
    state: localWidgetState ? Object.fromEntries(
      Object.entries(localWidgetState).filter(
        ([k]) => k !== MODEL_CONTEXT_KEY
      )
    ) : null,
    setState,
    ...
    hostInfo: mcpAppsHostInfo ?? void 0,
    hostCapabilities: mcpAppsHostCapabilities ?? void 0
  };
}
...
// src/react/ThemeProvider.tsx
var ThemeProvider = /* @__PURE__ */ __name(({ children, colorScheme = false }) => {
  const { theme, isAvailable } = useWidget();
  const [systemPreference, setSystemPreference] = (0, import_react6.useState)(
    () => {
...
function WidgetControls({
  ...
  const [isOverlayOpen, setIsOverlayOpen] = (0, import_react7.useState)(false);
  const containerRef = (0, import_react7.useRef)(null);
  const overlayRef = (0, import_react7.useRef)(null);
  const [windowOpenAiKeys, setWindowOpenAiKeys] = (0, import_react7.useState)([]);
  const [actionResult, setActionResult] = (0, import_react7.useState)("");
  const [toolName, setToolName] = (0, import_react7.useState)("get-my-city");
  const [toolArgs, setToolArgs] = (0, import_react7.useState)("{}");
  const [followUpMessage, setFollowUpMessage] = (0, import_react7.useState)(
    "Test follow-up message"
  );
  const [externalUrl, setExternalUrl] = (0, import_react7.useState)(
  ...
  const IconButton = /* @__PURE__ */ __name(({
    ...
    const [isButtonHovered, setIsButtonHovered] = (0, import_react7.useState)(false);
    ...
    return /* @__PURE__ */ import_react7.default.createElement(
      ...
      showLabels && /* @__PURE__ */ import_react7.default.createElement(
        "span",
        {
  ...
  return /* @__PURE__ */ import_react7.default.createElement(import_react7.default.Fragment, null, /* @__PURE__ */ import_react7.default.createElement(
    ...
    /* @__PURE__ */ import_react7.default.createElement(
      ...
      !isInInspector && /* @__PURE__ */ import_react7.default.createElement(import_react7.default.Fragment, null, !isFullscreen && !isPip && /* @__PURE__ */ import_react7.default.createElement(import_react7.default.Fragment, null, (viewControls === true || viewControls === "fullscreen") && /* @__PURE__ */ import_react7.default.createElement(IconButton, { onClick: handleFullscreen, label: "Fullscreen" }, /* @__PURE__ */ import_react7.default.createElement("path", { d: "M15 3h6v6" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "m21 3-7 7" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "m3 21 7-7" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "M9 21H3v-6" })), (viewControls === true || viewControls === "pip") && /* @__PURE__ */ import_react7.default.createElement(IconButton, { onClick: handlePip, label: "Picture in Picture" }, /* @__PURE__ */ import_react7.default.createElement("path", { d: "M21 9V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v10c0 1.1.9 2 2 2h4" }), /* @__PURE__ */ import_react7.default.createElement("rect", { width: "10", height: "7", x: "12", y: "13", rx: "2" }))), enableDebugger && /* @__PURE__ */ import_react7.default.createElement(IconButton, { onClick: handleToggleOverlay, label: "Debug Info" }, /* @__PURE__ */ import_react7.default.createElement("path", { d: "M12 20v-9" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "M14 7a4 4 0 0 1 4 4v3a6 6 0 0 1-12 0v-3a4 4 0 0 1 4-4z" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "M14.12 3.88 16 2" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "M21 21a4 4 0 0 0-3.81-4" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "M21 5a4 4 0 0 1-3.55 3.97" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "M22 13h-4" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "M3 21a4 4 0 0 1 3.81-4" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "M3 5a4 4 0 0 0 3.55 3.97" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "M6 13H2" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "m8 2 1.88 1.88" }), /* @__PURE__ */ import_react7.default.createElement("path", { d: "M9 7.13V6a3 3 0 1 1 6 0v1.13" })))
    ),
    children
  ), isOverlayOpen && enableDebugger && /* @__PURE__ */ import_react7.default.createElement(
    "div",
    {
      ref: overlayRef,
    ...
    /* @__PURE__ */ import_react7.default.createElement("div", { className: "max-w-[1200px] mx-auto pt-10" }, /* @__PURE__ */ import_react7.default.createElement("h1", { className: "text-lg font-bold mb-4 border-b border-gray-700 pb-2" }, "Debug Info"), /* @__PURE__ */ import_react7.default.createElement("table", { className: "w-full border-collapse border-spacing-0" }, /* @__PURE__ */ import_react7.default.createElement("tbody", null, /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "Props"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 whitespace-pre-wrap break-all" }, formatValue(props))), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "Output"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 whitespace-pre-wrap break-all" }, formatValue(output))), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "Metadata"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 whitespace-pre-wrap break-all" }, formatValue(metadata))), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "State"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 whitespace-pre-wrap break-all" }, formatValue(state))), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "Theme"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2" }, theme)), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "Display Mode"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2" }, displayMode)), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "Locale"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2" }, locale)), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "Max Height"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2" }, maxHeight, "px")), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "User Agent"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2" }, formatUserAgent(userAgent))), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "Safe Area"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2" }, formatSafeArea(safeArea))), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "API Available"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2" }, isAvailable ? "Yes" : "No")), /* @__PURE__ */ import_react7.default.createElement("tr", { className: "border-b border-gray-700" }, /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2 font-bold w-[200px] align-top" }, "window.openai Keys"), /* @__PURE__ */ import_react7.default.createElement("td", { className: "p-2" }, windowOpenAiKeys.length > 0 ? windowOpenAiKeys.join(", ") : "N/A")))), /* @__PURE__ */ import_react7.default.createElement("h2", { className: "text-base font-bold mt-8 mb-4 border-b border-gray-700 pb-2" }, "Actions"), /* @__PURE__ */ import_react7.default.createElement("div", { className: "flex flex-col gap-3" }, /* @__PURE__ */ import_react7.default.createElement("div", { className: "flex gap-2 items-center" }, /* @__PURE__ */ import_react7.default.createElement(
      "input",
      {
        type: "text",
        value: toolName,
      ...
      "Call Tool"
    )), /* @__PURE__ */ import_react7.default.createElement("div", { className: "flex gap-2 items-center" }, /* @__PURE__ */ import_react7.default.createElement(
      "input",
      {
        type: "text",
      ...
      "Open Link"
    )), /* @__PURE__ */ import_react7.default.createElement("div", { className: "flex gap-2 items-center" }, /* @__PURE__ */ import_react7.default.createElement("span", { className: "w-[150px] text-xs" }, "Display Mode:"), /* @__PURE__ */ import_react7.default.createElement(
      "button",
      {
    ...
    )), actionResult && /* @__PURE__ */ import_react7.default.createElement("div", { className: "mt-2 p-2 bg-[#1a1a1a] border border-gray-700 rounded whitespace-pre-wrap break-all text-[11px] max-h-[200px] overflow-auto" }, /* @__PURE__ */ import_react7.default.createElement("div", { className: "font-bold mb-1 text-gray-400" }, "Result:"), actionResult, /* @__PURE__ */ import_react7.default.createElement(
      "button",
      {
...
function McpUseProvider({
  ...
  const [containerElement, setContainerElement] = (0, import_react8.useState)(null);
  const lastHeightRef = (0, import_react8.useRef)(0);
  const debounceTimeoutRef = (0, import_react8.useRef)(null);
  const notificationInProgressRef = (0, import_react8.useRef)(false);
  const notifyHeight = (0, import_react8.useCallback)((height) => {
    if (typeof window === "undefined") return;
  ...
  const debouncedNotifyHeight = (0, import_react8.useCallback)(
    (height) => {
  ...
  content = /* @__PURE__ */ import_react8.default.createElement(ErrorBoundary, null, content);
  if (enableDebugger || viewControls) {
    content = /* @__PURE__ */ import_react8.default.createElement(WidgetControls, { debugger: enableDebugger, viewControls }, content);
  }
  content = /* @__PURE__ */ import_react8.default.createElement(ThemeProvider, { colorScheme }, content);
  if (autoSize) {
    const containerStyle = {
      width: "100%",
      minHeight: 0
    };
    content = /* @__PURE__ */ import_react8.default.createElement("div", { ref: setContainerElement, style: containerStyle }, content);
  }
  return /* @__PURE__ */ import_react8.default.createElement(import_react8.StrictMode, null, content);
}
...
function useFiles() {
  const isSupported = (0, import_react9.useMemo)(() => {
    ...
    return typeof window.openai?.uploadFile === "function" && typeof window.openai?.getFileDownloadUrl === "function";
  }, []);
  const upload = (0, import_react9.useMemo)(
    () => async (file, options = {}) => {
      if (!isSupported) {
        throw new Error(
          "[useFiles] File upload is not supported in this host. Check `isSupported` before calling `upload`. File operations are only available in the ChatGPT Apps SDK environment."
        );
      }
      const metadata = await window.openai.uploadFile(
        file
      );
      ...
      if (modelVisible && window.openai?.setWidgetState) {
        ...
        window.openai.setWidgetState({ ...prev, imageIds }).catch((err) => {
  ...
  const getDownloadUrl = (0, import_react9.useMemo)(
    () => async (file) => {
      if (!isSupported) {
        throw new Error(
          "[useFiles] File download is not supported in this host. Check `isSupported` before calling `getDownloadUrl`. File operations are only available in the ChatGPT Apps SDK environment."
        );
      }
...
__name(useFiles, "useFiles");

// src/react/useCallTool.ts
var import_react10 = require("react");
function useCallTool(name) {
  const [{ status, data, error }, setCallToolState] = (0, import_react10.useState)({ status: "idle", data: void 0, error: void 0 });
  const callIdRef = (0, import_react10.useRef)(0);
  ...
  const callToolAsync = (0, import_react10.useCallback)(
    ((args) => {
      if (args === void 0) {
        return execute(null);
      }
...
var McpClientContext = (0, import_react11.createContext)(null);
...
function McpServerWrapper({
  ...
  const mcpOptions = (0, import_react11.useMemo)(() => {
    const {
      name: _name,
  ...
  const combinedWrapTransport = (0, import_react11.useMemo)(() => {
  ...
  const [pendingSamplingRequests, setPendingSamplingRequests] = (0, import_react11.useState)([]);
  const samplingIdCounter = (0, import_react11.useRef)(0);
  const [pendingElicitationRequests, setPendingElicitationRequests] = (0, import_react11.useState)([]);
  const elicitationIdCounter = (0, import_react11.useRef)(0);
  const markNotificationRead = (0, import_react11.useCallback)((notificationId) => {
    setNotifications(
      (prev) => prev.map((n) => n.id === notificationId ? { ...n, read: true } : n)
    );
  }, []);
  const markAllNotificationsRead = (0, import_react11.useCallback)(() => {
    setNotifications((prev) => prev.map((n) => ({ ...n, read: true })));
  }, []);
  const clearNotifications = (0, import_react11.useCallback)(() => {
    setNotifications([]);
  }, []);
  const handleNotification = (0, import_react11.useCallback)(
    (notification) => {
  ...
  const approveSampling = (0, import_react11.useCallback)(
    (requestId, result) => {
  ...
  const rejectSampling = (0, import_react11.useCallback)((requestId, error) => {
    setPendingSamplingRequests((prev) => {
      const request = prev.find((r) => r.id === requestId);
      if (request) {
  ...
  const samplingCallback = (0, import_react11.useCallback)(
    async (params) => {
      return new Promise((resolve, reject2) => {
  ...
  const approveElicitation = (0, import_react11.useCallback)(
    (requestId, result) => {
  ...
  const rejectElicitation = (0, import_react11.useCallback)((requestId, error) => {
    setPendingElicitationRequests((prev) => {
      const request = prev.find((r) => r.id === requestId);
      if (request) {
  ...
  const elicitationCallback = (0, import_react11.useCallback)(
    async (params) => {
      return new Promise((resolve, reject2) => {
  ...
  const publicSamplingRequests = (0, import_react11.useMemo)(
  ...
  const publicElicitationRequests = (0, import_react11.useMemo)(
  ...
  const unreadNotificationCount = (0, import_react11.useMemo)(
    () => notifications.filter((n) => !n.read).length,
    [notifications]
  );
  const toolsFingerprint = (0, import_react11.useMemo)(() => {
  ...
  const resourcesFingerprint = (0, import_react11.useMemo)(
    () => JSON.stringify(
      mcp.resources.map((r) => ({
        uri: r.uri,
        name: r.name,
  ...
  const promptsFingerprint = (0, import_react11.useMemo)(
    () => JSON.stringify(
      mcp.prompts.map((p) => ({
        name: p.name,
  ...
  const onUpdateRef = (0, import_react11.useRef)(onUpdate);
  const prevServerRef = (0, import_react11.useRef)(null);
  const prevFingerprintsRef = (0, import_react11.useRef)({ tools: "", resources: "", prompts: "" });
...
function McpClientProvider({
  ...
  const [serverConfigs, setServerConfigs] = (0, import_react11.useState)([]);
  const [servers, setServers] = (0, import_react11.useState)([]);
  const [storageLoaded, setStorageLoaded] = (0, import_react11.useState)(false);
  const cachedMetadataRef = (0, import_react11.useRef)({});
  const [rpcWrapTransport, setRpcWrapTransport] = (0, import_react11.useState)(void 0);
  const [rpcLoggingReady, setRpcLoggingReady] = (0, import_react11.useState)(false);
  (0, import_react11.useEffect)(() => {
  ...
  const handleServerUpdate = (0, import_react11.useCallback)(
    (updatedServer) => {
      providerLogger.debug(
  ...
  const addServer = (0, import_react11.useCallback)((id, options) => {
  ...
  const removeServer = (0, import_react11.useCallback)(
    (id) => {
      setServers((prev) => {
  ...
  const updateServer = (0, import_react11.useCallback)(
    async (id, options) => {
  ...
  const updateServerMetadata = (0, import_react11.useCallback)(
    async (id, metadata) => {
      return new Promise((resolve) => {
        const currentConfig = serverConfigs.find((s) => s.id === id);
        if (!currentConfig) {
        ...
        setServers(
          (prev) => prev.map(
            (server) => server.id === id ? { ...server, name: metadata.name } : server
          )
        );
        setServerConfigs((prev) => {
          const updated = prev.map(
  ...
  const getServer = (0, import_react11.useCallback)(
    (id) => {
  ...
  const { capabilities: defaultCapabilities, ...clientInfoWithoutCaps } = clientInfo || {};
  const clientInfoForWrapper = (0, import_react11.useMemo)(
    () => Object.keys(clientInfoWithoutCaps).length ? clientInfoWithoutCaps : void 0,
    [clientInfo]
  );
  const mergedServerConfigs = (0, import_react11.useMemo)(
  ...
  return /* @__PURE__ */ import_react11.default.createElement(McpClientContext.Provider, { value: contextValue }, children, mergedServerConfigs.map((config) => /* @__PURE__ */ import_react11.default.createElement(
    McpServerWrapper,
    {
...
function useMcpClient() {
  const context = (0, import_react11.useContext)(McpClientContext);
  if (!context) {
