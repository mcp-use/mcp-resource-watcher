  ...
  "src/logging.ts"() {
    ...
    Logger = class {
      ...
      static configure(options = {}) {
        ...
        Object.values(this.instances).forEach((logger3) => {
          logger3.level = resolvedLevel;
          logger3.setFormat(format);
        });
      }
...
var telFetch;
var init_tel_fetch = __esm({
  "src/telemetry/tel-fetch.ts"() {
    "use strict";
    telFetch = /* @__PURE__ */ __name(async (url, options) => {
      try {
      ...
      return {
        status: 200,
        text: /* @__PURE__ */ __name(() => Promise.resolve(""), "text"),
        json: /* @__PURE__ */ __name(() => Promise.resolve({}), "json")
      };
    }, "telFetch");
  }
});
...
var cachedEnvironment, ScarfEventLogger, Telemetry, Tel;
var init_telemetry_node = __esm({
  "src/telemetry/telemetry-node.ts"() {
    "use strict";
    init_runtime();
    init_logging();
...
var proxy_client_exports = {};
__export(proxy_client_exports, {
  mountSession: () => mountSession
});
function jsonSchemaToZod(schema) {
  if (!schema || typeof schema !== "object") return import_zod.z.any();
  ...
  if (schema.type === "string") return import_zod.z.string();
  if (schema.type === "number" || schema.type === "integer") return import_zod.z.number();
  if (schema.type === "boolean") return import_zod.z.boolean();
  if (schema.type === "array") {
    return schema.items ? import_zod.z.array(jsonSchemaToZod(schema.items)) : import_zod.z.array(import_zod.z.any());
  }
      ...
      parentServer.prompt(
        {
          ...
          schema: import_zod.z.object(
            prompt.arguments?.reduce(
              (acc, arg) => {
                let propSchema = import_zod.z.string();
                if (arg.description) {
...
function buildWidgetUrl(widget2, props, config) {
  const slugifiedWidget = slugifyWidgetName(widget2);
  const url = new URL(
...
async function createRemoteDomResource(uri, script, framework = "react", encoding = "text", _adapters, _metadata) {
  ...
  if (encoding === "blob") {
    resource2.blob = Buffer.from(script).toString("base64");
  } else {
    resource2.text = script;
  }
  return { type: "resource", resource: resource2 };
}
...
var import_server;
var init_mcp_ui_adapter = __esm({
  "src/server/widgets/mcp-ui-adapter.ts"() {
    "use strict";
    import_server = require("@mcp-ui/server");
    init_widget_helpers();
...
var widget_helpers_exports = {};
__export(widget_helpers_exports, {
  applyDefaultProps: () => applyDefaultProps,
  convertPropsToInputs: () => convertPropsToInputs,
  createWidgetRegistration: () => createWidgetRegistration,
  createWidgetUIResource: () => createWidgetUIResource,
  ensureWidgetMetadata: () => ensureWidgetMetadata,
  generateWidgetUri: () => generateWidgetUri,
  getContentType: () => getContentType,
  processWidgetHtml: () => processWidgetHtml,
  readBuildManifest: () => readBuildManifest,
  readWidgetHtml: () => readWidgetHtml,
  registerWidgetFromTemplate: () => registerWidgetFromTemplate,
  setupFaviconRoute: () => setupFaviconRoute,
  setupPublicRoutes: () => setupPublicRoutes,
  slugifyWidgetName: () => slugifyWidgetName
});
...
var import_server2, McpAppsAdapter;
var init_mcp_apps = __esm({
  "src/server/widgets/adapters/mcp-apps.ts"() {
    "use strict";
    import_server2 = require("@modelcontextprotocol/ext-apps/server");
    init_base();
    McpAppsAdapter = class extends BaseProtocolAdapter {
      static {
      ...
      buildToolMetadata(definition, uri) {
        const meta = {
          ...
          [import_server2.RESOURCE_URI_META_KEY]: uri
        };
        return meta;
      }
      /**
...
var AppsSdkAdapter;
var init_apps_sdk = __esm({
  "src/server/widgets/adapters/apps-sdk.ts"() {
    "use strict";
    init_base();
    AppsSdkAdapter = class extends BaseProtocolAdapter {
      static {
...
var protocol_helpers_exports = {};
__export(protocol_helpers_exports, {
  buildDualProtocolMetadata: () => buildDualProtocolMetadata,
  buildResourceUiMeta: () => buildResourceUiMeta,
  copyMetadataFields: () => copyMetadataFields,
  createProtocolAdapters: () => createProtocolAdapters,
  deepMergeUiMetadata: () => deepMergeUiMetadata,
  detectSchemaType: () => detectSchemaType,
  generateToolOutput: () => generateToolOutput,
  generateUniqueWidgetUri: () => generateUniqueWidgetUri,
  getBuildIdPart: () => getBuildIdPart,
  getResourceUriAndMimeType: () => getResourceUriAndMimeType
});
...
var init_protocol_helpers = __esm({
  "src/server/widgets/protocol-helpers.ts"() {
    "use strict";
    init_mcp_apps();
    init_apps_sdk();
...
function zodToTypeString(schema) {
  ...
  switch (normalizedType) {
    ...
    case "ZodEnum": {
      const values = Array.isArray(def.values) ? def.values : def.entries ? Object.values(def.entries) : void 0;
...
var tool_registry_generator_exports = {};
__export(tool_registry_generator_exports, {
  generateToolRegistryTypes: () => generateToolRegistryTypes
});
...
export {};
`;
    const mcpUseDir = (0, import_node_path.join)(projectRoot, MCP_USE_DIR);
    const outputPath = (0, import_node_path.join)(mcpUseDir, TOOL_REGISTRY_FILENAME);
    let shouldWrite = true;
    try {
      const existingContent = await (0, import_promises.readFile)(outputPath, "utf-8");
    ...
    if (shouldWrite) {
      await (0, import_promises.mkdir)(mcpUseDir, { recursive: true });
      await (0, import_promises.writeFile)(outputPath, content, "utf-8");
      console.log(
...
var import_promises, import_node_path, TOOL_REGISTRY_FILENAME, MCP_USE_DIR;
var init_tool_registry_generator = __esm({
  "src/server/utils/tool-registry-generator.ts"() {
    "use strict";
    import_promises = require("fs/promises");
    import_node_path = require("path");
    init_zod_to_ts();
    TOOL_REGISTRY_FILENAME = "tool-registry.d.ts";
    MCP_USE_DIR = ".mcp-use";
...
var import_node_async_hooks, STORAGE_KEY, requestContextStorage;
var init_context_storage = __esm({
  "src/server/context-storage.ts"() {
    "use strict";
    import_node_async_hooks = require("async_hooks");
    STORAGE_KEY = "__mcp_use_request_context_storage__";
    requestContextStorage = globalThis[STORAGE_KEY] ?? (globalThis[STORAGE_KEY] = new import_node_async_hooks.AsyncLocalStorage());
...
__export(tool_execution_helpers_exports, {
  ...
  buildHandlerContext: () => buildHandlerContext,
  createClientCapabilityChecker: () => createClientCapabilityChecker,
  ...
  normalizeUserContext: () => normalizeUserContext,
...
async function withTimeout(promise, timeout, errorMessage) {
  if (timeout && timeout !== Infinity) {
    const timeoutPromise = new Promise((_, reject2) => {
      setTimeout(() => reject2(new Error(errorMessage)), timeout);
    });
...
function createLogMethod(sendNotification2, minLogLevel) {
  ...
  return async (level, message, logger3) => {
...
function createEnhancedContext(baseContext, createMessage, elicitInput, progressToken, sendNotification2, minLogLevel, clientCapabilities, sessionId, sessions, clientInfo, requestMeta) {
...
var import_zod_json_schema_compat, LOG_LEVELS, VALID_LOG_LEVELS, MCP_UI_EXTENSION_ID, MCP_UI_MIME_TYPE;
...
`;
          ...
          if (execution.exitCode !== 0) {
            error2 = execution.stderr || "Execution failed";
          } else {
          ...
          if (error2 && (error2.includes("timeout") || error2.includes("timed out"))) {
            error2 = "Script execution timed out";
          }
        }
        return {
          result,
      ...
      async cleanup() {
        if (this.codeExecSandbox) {
          try {
            ...
            logger.error("Failed to stop E2B code execution sandbox:", error2);
          }
        }
      }
    };
  ...
  "src/client/executors/vm.ts"() {
    ...
    VMCodeExecutor = class extends BaseCodeExecutor {
      ...
      `;
          ...
          error2 = e.message || String(e);
          if (e.code === "ERR_SCRIPT_EXECUTION_TIMEOUT" || e.message === "Script execution timed out." || typeof error2 === "string" && (error2.includes("timed out") || error2.includes("timeout"))) {
            error2 = "Script execution timed out";
          }
          if (e.stack) {
...
var import_types2, BaseConnector;
    ...
    BaseConnector = class {
      ...
      setupRootsHandler() {
        ...
        this.client.setRequestHandler(
          import_types2.ListRootsRequestSchema,
      ...
      setupSamplingHandler() {
        ...
        this.client.setRequestHandler(
          import_types2.CreateMessageRequestSchema,
      ...
      async initialize(defaultRequestOptions = this.opts.defaultRequestOptions ?? {}) {
        ...
        try {
          ...
          const error2 = err;
          if (error2.code === -32601) {
            logger.debug("Server does not implement tools/list, assuming no tools");
          } else {
            logger.debug("Failed to list tools, assuming empty:", error2.message);
          }
          this.toolsCache = [];
        }
      ...
      async establishConnection() {
        ...
        transport.send = async (message) => {
          ...
          try {
            ...
            if (error2?.code === 404 && transport.sessionId && !this.reinitializing) {
              logger.warn(
...
var import_zod4, JSONSchemaToZod;
  ...
  "src/utils/json-schema-to-zod/JSONSchemaToZod.ts"() {
    ...
    JSONSchemaToZod = class {
      ...
      static createUnionFromTypes(types, baseSchema) {
        ...
        return import_zod4.z.union(schemas);
      }
      /**
      ...
      static parseNumberSchema(schema) {
        const numberSchema = import_zod4.z.number();
      ...
      static parseString(schema) {
        const stringSchema = import_zod4.z.string();
      ...
      static parseArray(schema) {
        if (Array.isArray(schema.items)) {
          ...
          return import_zod4.z.union(tupleSchemas);
        }
        const itemSchema = schema.items ? this.parseSchema(schema.items) : import_zod4.z.any();
        const arraySchema = import_zod4.z.array(itemSchema);
      ...
      static parseObject(schema) {
        ...
        if (!schema.properties && schema.additionalProperties && typeof schema.additionalProperties === "object") {
          ...
          return import_zod4.z.object({}).catchall(valueSchema);
        }
        const shape = {};
        this.processObjectProperties(schema, shape);
        return this.processAdditionalProperties(schema, import_zod4.z.object(shape));
      }
      /**
      ...
      static createBaseObjectSchema(schema) {
        ...
        const zodObject = import_zod4.z.object(shape);
      ...
      static createUnionFromSchemas(schemas) {
        ...
        for (const subSchema of schemas) {
          if (subSchema.type === "null") {
            zodSchemas.push(import_zod4.z.null());
          } else {
            zodSchemas.push(this.parseSchema(subSchema));
          }
        }
        if (zodSchemas.length >= 2) {
          return import_zod4.z.union(zodSchemas);
...
var import_node_fs2, import_node_path4, MCPClient;
...
__export(server_exports, {
  ...
  RESOURCE_MIME_TYPE: () => import_server3.RESOURCE_MIME_TYPE,
  RESOURCE_URI_META_KEY: () => import_server3.RESOURCE_URI_META_KEY,
  ...
  createMcpAppsResource: () => createMcpAppsResource,
  ...
  mix: () => mix,
  mountMcpProxy: () => mountMcpProxy,
  mountOAuthProxy: () => mountOAuthProxy,
  oauthAuth0Provider: () => oauthAuth0Provider,
  oauthBetterAuthProvider: () => oauthBetterAuthProvider,
  oauthClerkProvider: () => oauthClerkProvider,
  ...
  supportsApps: () => supportsApps,
  text: () => text,
  titledEnum: () => titledEnum,
  titledMultiEnum: () => titledMultiEnum,
  untitledEnum: () => untitledEnum,
  untitledMultiEnum: () => untitledMultiEnum,
  widget: () => widget,
  xml: () => xml
});
...
async function startServer(app, port, host, options) {
  if (isDeno) {
    ...
    return {
      ...
      forceClose: /* @__PURE__ */ __name(async () => {
      }, "forceClose")
    };
  } else {
    ...
    return {
      close: /* @__PURE__ */ __name(() => new Promise((resolve, reject2) => {
        nodeServer.close((err) => err ? reject2(err) : resolve());
      }), "close"),
      forceClose: /* @__PURE__ */ __name(() => new Promise((resolve) => {
...
function isExpressMiddleware(middleware) {
  ...
  if (paramCount === 2) {
    ...
    const hasHonoPattern = honoPatterns.some(
      (pattern) => pattern.test(fnString)
    );
    if (hasHonoPattern) {
      return false;
    ...
    get(target2, prop) {
      if (prop === "use") {
        return async (...args) => {
          ...
          if (hasPath && args[0].startsWith("mcp:")) {
            const pattern = args[0].slice(4);
            const handlers2 = args.slice(1);
            if (typeof target2._registerMcpMiddleware === "function") {
              for (const h of handlers2) {
                target2._registerMcpMiddleware(pattern, h);
              }
            }
            return target2;
          }
          ...
          if (hasPath) {
            return app.use(path2, ...handlers);
          } else {
            return app.use(...handlers);
          }
        };
      }
      if (typeof prop === "string" && HMR_HTTP_METHODS.has(prop)) {
        return (path2, ...handlers) => {
          ...
          if (!customRoutes) {
            return app[prop](path2, ...handlers);
          }
          const key = `${prop}:${path2}`;
...
var import_completable = require("@modelcontextprotocol/sdk/server/completable.js");
function completable(schema, complete) {
  ...
  return (0, import_completable.completable)(schema, callback);
}
...
async function computeProjectSrcSourceLine(widgetTempDir) {
  const projectSrcPath = pathHelpers.join(getCwd(), "src");
  if (!await fsHelpers.existsSync(projectSrcPath)) return "";
  const relative = pathHelpers.relative(widgetTempDir, projectSrcPath).replace(/\\/g, "/");
  return `@source "${relative}";
`;
}
__name(computeProjectSrcSourceLine, "computeProjectSrcSourceLine");
...
async function mountWidgetsDev(app, serverConfig, registerWidget, updateWidgetTool, removeWidgetTool, options) {
  ...
  const resourcesDir = options?.resourcesDir || process.env.MCP_USE_WIDGETS_DIR || "resources";
  ...
  const { slugifyWidgetName: slugifyWidgetName2 } = await Promise.resolve().then(() => (init_widget_helpers(), widget_helpers_exports));
...
import './styles.css'
import Component from '${widget2.entry.replace(/\\/g, "/")}'
  ...
  const watchResourcesPlugin = {
    ...
    configureServer(server) {
      ...
      server.watcher.on("unlink", async (filePath) => {
        ...
        const { slugifyWidgetName: slugifyWidgetName3 } = await Promise.resolve().then(() => (init_widget_helpers(), widget_helpers_exports));
        if ((relativePath.endsWith(".tsx") || relativePath.endsWith(".ts")) && !relativePath.includes("/")) {
          ...
          try {
            await fs2.access(widgetDir);
            await fs2.rm(widgetDir, { recursive: true, force: true });
            console.log(
  ...
  <body>
    ...
    <script type="module" src="${fullBaseUrl}/${slugifiedName}/entry.tsx"></script>
      ...
      const extractAndRegisterWidget = /* @__PURE__ */ __name(async (widgetName, entryPath, changedFilePath, isHmrUpdate = false) => {
        ...
        if (isHmrUpdate) {
          const schemaField = metadata.props || metadata.inputs;
          const { slugifyWidgetName: slugifyWidgetName4, processWidgetHtml: processWidgetHtml2 } = await Promise.resolve().then(() => (init_widget_helpers(), widget_helpers_exports));
          const { buildDualProtocolMetadata: buildDualProtocolMetadata2, getBuildIdPart: getBuildIdPart2 } = await Promise.resolve().then(() => (init_protocol_helpers(), protocol_helpers_exports));
          const widgetType = metadata.appsSdkMetadata && !metadata.metadata ? "appsSdk" : "mcpApps";
          ...
          const hmrDefinition = {
            ...
            type: widgetType,
            description,
            metadata: metadata.metadata ? enrichedCspMetadata : void 0
          };
        ...
        const metadataToRegister = {
          ...metadata,
          metadata: metadata.metadata ? enrichedCspMetadata ?? metadata.metadata : void 0,
        ...
        if (isWidgetFile) {
          ...
          if (widget2) {
            try {
              ...
              Promise.resolve().then(() => (init_tool_registry_generator(), tool_registry_generator_exports)).then(
...
`;
  ...
  const serverOnlyGuard = {
    ...
    resolveId(id, importer) {
      ...
      throw new Error(
        `Widget imports "${id}"${from}, which is a Next.js server-only module. Widgets run in a browser iframe and cannot use server APIs.

To fix:
  \u2022 Remove the import from the widget (or from any module the widget transitively imports)
  \u2022 If the widget needs data from ${id}, read it inside an MCP tool and pass the result through the widget's props`
      );
    }
  };
  ...
  const viteServer = await createServer({
    ...
    resolve: {
      ...
      ...hasProjectTsconfig ? { tsconfigPaths: true } : { alias: { "@": pathHelpers.join(getCwd(), resourcesDir) } }
    },
    build: {
  ...
  const warmedWidgetEntries = /* @__PURE__ */ new Set();
  warmWidgetEntry.current = async (widgetName, _reason) => {
    const slugifiedName = slugifyWidgetName2(widgetName);
    ...
    try {
      ...
      console.warn(`[WIDGETS] Failed to warm widget entry ${entryUrl}:`, error2);
    }
  };
...
` + Object.entries(req.headers).map(([k, v]) => `${k}: ${v}`).join("\r\n") + "\r\n\r\n";
  ...
  for (const widget2 of widgets) {
    ...
    const slugifiedName = slugifyWidgetName2(widget2.name);
    await warmWidgetEntry.current(widget2.name, "initial-registration");
    await registerWidgetFromTemplate(
      widget2.name,
      pathHelpers.join(tempDir, slugifiedName, "index.html"),
      metadataToRegister,
...
function uiResourceRegistration(server, definition) {
  ...
  if (exposeAsTool) {
    ...
    const isZodSchema = propsOrSchema && typeof propsOrSchema === "object" && propsOrSchema instanceof import_zod2.default.ZodObject;
...
async function mountInspectorUI(app, serverHost, serverPort, isProduction) {
  ...
  try {
    ...
    mountInspector(app, {
      ...
      serverPort: typeof serverPort === "number" ? serverPort : void 0
    });
    console.log(
...
function registerPrompt(promptDefinition, callback) {
  ...
  const wrappedCallback = /* @__PURE__ */ __name(async (params, extra) => {
    ...
    const { findSessionContext: findSessionContext2, createClientCapabilityChecker: createClientCapabilityChecker2 } = await Promise.resolve().then(() => (init_tool_execution_helpers(), tool_execution_helpers_exports));
    ...
    Object.defineProperty(enhancedContext, "client", {
      value: createClientCapabilityChecker2(
        session?.clientCapabilities,
        session?.clientInfo
      ),
      writable: true,
...
function convertZodSchemaToParams(zodSchema) {
  ...
  if (!hasZodObjectStructure && !(zodSchema instanceof import_zod3.z.ZodObject)) {
...
function toolRegistration(toolDefinition, callback) {
  ...
  this.server.registerTool(
    ...
    async (params, extra) => {
      ...
      const requestMeta = extra?._meta && Object.keys(extra._meta).some((k) => k !== "progressToken") ? Object.fromEntries(
...
__name(getManufactInspectorUrl, "getManufactInspectorUrl");
var LOGO_SVG_PATHS = {
...
__name(getMeshGradientScript, "getMeshGradientScript");
function generateLandingPage(name, version, url, description, tools, prompts, resources, iconUrl) {
  ...
  const metaDescription = safeDescription || `${safeName} MCP Server \u2014 Connect with Claude Code, Cursor, and VS Code. Model Context Protocol server.`;
  ...
  const normalizedTools = tools && tools.length > 0 ? tools.map(
    (t) => typeof t === "string" ? { name: t } : { name: t.name, description: t.description }
  ) : [];
  ...
  const promptsListHtml = prompts && prompts.length > 0 ? prompts.map(
    (p) => `<div class="spec-item"><code class="spec-name">${escapeHtml(p.name)}</code>${p.description ? `<p class="spec-desc">${escapeHtml(p.description)}</p>` : ""}</div>`
  ).join("\n") : "";
  const resourcesListHtml = resources && resources.length > 0 ? resources.map(
    (r) => `<div class="spec-item"><code class="spec-uri">${escapeHtml(r.uri)}</code>${r.name ? `<strong>${escapeHtml(r.name)}</strong>` : ""}${r.description ? `<p class="spec-desc">${escapeHtml(r.description)}</p>` : ""}</div>`
  ).join("\n") : "";
  return `<!DOCTYPE html>
<html lang="en">
<head>
...
function isJsonRpcRequest(msg) {
  return !!msg && typeof msg === "object" && "id" in msg && "method" in msg && !("result" in msg) && !("error" in msg);
}
...
var FileSystemSessionStore = class {
  ...
  pendingSave = false;
  constructor(config = {}) {
    this.filePath = config.path ?? (0, import_node_path2.join)(process.cwd(), ".mcp-use", "sessions.json");
  ...
  async performSave() {
    ...
    try {
      const dir = (0, import_node_path2.dirname)(this.filePath);
      ...
      await (0, import_promises2.writeFile)(tempPath, JSON.stringify(data, null, 2), "utf-8");
      await (0, import_promises2.rename)(tempPath, this.filePath);
    } catch (error2) {
      console.error(
      ...
      try {
        ...
        if ((0, import_node_fs.existsSync)(tempPath)) {
          await (0, import_promises2.unlink)(tempPath);
        }
      } catch {
      }
    } finally {
...
var RedisStreamManager = class {
  ...
  async create(sessionId, controller) {
    try {
      if (this.localControllers.has(sessionId)) {
        ...
        const channel2 = this.getChannel(sessionId);
        const deleteChannel2 = `delete:${channel2}`;
        try {
          await this.pubSubClient.unsubscribe?.(channel2);
          await this.pubSubClient.unsubscribe?.(deleteChannel2);
        } catch {
        }
      }
  ...
  getRequestRouteKey(sessionId, requestId) {
   ...
   */
  async registerOutboundRequest(requestId, sessionId) {
   ...
   */
  async forwardInboundResponse(message, sessionId) {
  ...
  onForwardedResponse(handler) {
    ...
    this.pubSubClient.subscribe(serverChannel, (raw) => {
      try {
...
function getDebugLevel() {
  ...
  const debugEnabled = debugEnv !== void 0 && debugEnv !== "" && debugEnv !== "0" && debugEnv.toLowerCase() !== "false";
  return debugEnabled ? "trace" : "info";
}
__name(getDebugLevel, "getDebugLevel");
...
__name(inlineJson, "inlineJson");
function shortSessionId(sid) {
  if (!sid) return null;
  return sid.replace(/-/g, "").slice(0, 7);
}
__name(shortSessionId, "shortSessionId");
async function extractResponseError(res) {
  if (!res.body) return null;
  let text2;
  try {
    text2 = await res.clone().text();
  } catch {
    return null;
  }
  ...
  const payloads = [];
  const tryParse = /* @__PURE__ */ __name((raw) => {
    try {
  ...
  for (const payload of payloads) {
    for (const msg of Array.isArray(payload) ? payload : [payload]) {
      ...
      if (m.result?.isError === true) {
        const textBlock = Array.isArray(m.result.content) ? m.result.content.find(
...
__name(extractResponseError, "extractResponseError");
async function requestLogger(c, next) {
  ...
  const parts = [import_chalk2.default.gray(`[${timestamp}]`)];
  const sessPrefix = !isInitialize ? shortSessionId(incomingSid) : null;
  if (sessPrefix) parts.push(import_chalk2.default.gray(`sess=${sessPrefix}`));
  parts.push(method);
  parts.push(import_chalk2.default.bold(pathname));
  if (mcpMethod) {
    if (isInitialize) {
      const ci = requestBody?.params?.clientInfo;
      const label = ci?.name && ci?.version ? `: ${ci.name}/${ci.version}` : ci?.name ? `: ${ci.name}` : "";
      parts.push(import_chalk2.default.bold(`[initialize${label}]`));
    } else if (mcpMethod === "tools/call") {
      const toolName = requestBody?.params?.name ?? "?";
      let segment = import_chalk2.default.bold(`[tools/call: ${toolName}]`);
      if (level !== "info") {
      ...
      parts.push(import_chalk2.default.bold(`[resources/read: ${uri}]`));
    } else if (mcpMethod === "prompts/get") {
      const promptName = requestBody?.params?.name ?? "?";
      parts.push(import_chalk2.default.bold(`[prompts/get: ${promptName}]`));
    } else {
      parts.push(import_chalk2.default.bold(`[${mcpMethod}]`));
    }
  }
  if (isInitialize && responseSid) {
    parts.push(import_chalk2.default.gray(`\u2192 session=${shortSessionId(responseSid)}`));
  }
  let outcomePart;
  const errMsg = await extractResponseError(c.res);
  if (errMsg) {
    outcomePart = import_chalk2.default.red(`ERROR ${errMsg}`);
  } else if (mcpMethod) {
    outcomePart = statusCode >= 400 ? import_chalk2.default.red(`ERROR (HTTP ${statusCode})`) : import_chalk2.default.green("OK");
  } else {
    outcomePart = statusCode >= 500 ? import_chalk2.default.magenta(String(statusCode)) : statusCode >= 400 ? import_chalk2.default.red(String(statusCode)) : statusCode >= 300 ? import_chalk2.default.yellow(String(statusCode)) : import_chalk2.default.green(String(statusCode));
  }
  parts.push(outcomePart);
  parts.push(import_chalk2.default.gray(`(${durationMs}ms)`));
  console.log(parts.join(" "));
  if (level !== "trace") return;
  console.log("\n" + import_chalk2.default.cyan("=".repeat(80)));
  console.log(import_chalk2.default.bold.cyan("[TRACE] Request Details"));
  console.log(import_chalk2.default.cyan("-".repeat(80)));
  if (Object.keys(requestHeaders).length > 0) {
    console.log(import_chalk2.default.yellow("Request Headers:"));
    console.log(formatForLogging(requestHeaders));
  }
  if (requestBody !== null) {
    console.log(import_chalk2.default.yellow("Request Body:"));
  ...
  if (Object.keys(responseHeaders).length > 0) {
    console.log(import_chalk2.default.yellow("Response Headers:"));
    ...
    if (c.res.body !== null && c.res.body !== void 0) {
      try {
        ...
        if (responseBody !== null && responseBody.length > 0) {
          console.log(import_chalk2.default.yellow("Response Body:"));
          try {
...
... (truncated, ${responseBody.length - maxLength} more characters)`
          ...
          console.log(import_chalk2.default.yellow("Response Body:") + " (empty)");
        }
      } catch {
        console.log(import_chalk2.default.yellow("Response Body:") + " (unable to clone/read)");
      }
    } else {
      console.log(import_chalk2.default.yellow("Response Body:") + " (no body)");
    }
  } catch {
    console.log(import_chalk2.default.yellow("Response Body:") + " (unable to read)");
  }
  console.log(import_chalk2.default.cyan("=".repeat(80)) + "\n");
}
...
// src/server/endpoints/mount-mcp.ts
init_runtime();
function wrapTransportForStreamManager(transport, sessionId, streamManager) {
  const originalSend = transport.send.bind(transport);
...
__name(wrapTransportForStreamManager, "wrapTransportForStreamManager");
async function registerSseStream(transport, sessionId, streamManager) {
  const sseStreamId = transport._standaloneSseStreamId || "_GET_stream";
...
__name(registerSseStream, "registerSseStream");
async function maybeForwardResponses(request, sessionId, streamManager) {
  ...
  try {
    ...
    for (const msg of messages) {
      if (isJsonRpcResponse(msg)) {
        await streamManager.forwardInboundResponse(msg, sessionId);
      }
    }
  } catch {
  }
}
__name(maybeForwardResponses, "maybeForwardResponses");
async function mountMcp(app, mcpServerInstance, sessions, config, isProductionMode2) {
  ...
  const sessionStore = config.sessionStore ?? (isProductionMode2 ? new InMemorySessionStore() : new FileSystemSessionStore({
    path: (0, import_node_path3.join)(process.cwd(), ".mcp-use", "sessions.json")
  }));
  const streamManager = config.streamManager ?? new InMemoryStreamManager();
  const transports = /* @__PURE__ */ new Map();
  if (streamManager.onForwardedResponse) {
    streamManager.onForwardedResponse((message, sid) => {
  ...
  const handleRequest = /* @__PURE__ */ __name(async (c) => {
    if (c.req.method === "GET") {
      ...
      if (isBrowser) {
        ...
        let iconUrl;
        const iconSrc = instance.favicon ?? instance.config?.favicon ?? instance.config?.icons?.[0]?.src;
        if (iconSrc) {
          iconUrl = iconSrc.startsWith("http") ? iconSrc : `${origin}/mcp-use/public/${iconSrc.replace(/^\//, "")}`;
        }
        const regs = instance.registrations;
        const landingTools = regs?.tools && Array.from(regs.tools.entries()).map(([name, r]) => ({
          name,
          description: r.config.description
        }));
        const landingPrompts = regs?.prompts && Array.from(regs.prompts.entries()).map(([name, r]) => ({
          name,
          description: r.config.description || void 0
        }));
        const landingResources = regs?.resources && Array.from(regs.resources.values()).map((r) => ({
          uri: r.config.uri,
          name: r.config.name,
          description: r.config.description
        }));
        const landingPage = generateLandingPage(
          ...
          fullUrl,
          config.description,
          landingTools?.length ? landingTools : void 0,
          landingPrompts?.length ? landingPrompts : void 0,
          landingResources?.length ? landingResources : void 0,
          iconUrl
        );
    ...
    if (useStatelessMode) {
      ...
      try {
        ...
        if (!clientSupportsSSE) {
          ...
          return await runWithContext(
            c,
            async () => transport.handleRequest(modifiedRequest)
          );
        }
        return await runWithContext(
          c,
      ...
      if (sessionId && await sessionStore.has(sessionId) && !transports.has(sessionId)) {
        ...
        wrapTransportForStreamManager(transport2, sessionId, streamManager);
        await server2.connect(transport2);
        transport2._initialized = true;
        transport2.sessionId = sessionId;
        transports.set(sessionId, transport2);
        ...
        const recoveryResponse = await runWithContext(
          c,
          async () => transport2.handleRequest(c.req.raw),
          sessionId
        );
        if (c.req.method === "GET") {
          await registerSseStream(transport2, sessionId, streamManager);
        }
      ...
      const transport = new WebStandardStreamableHTTPServerTransport({
        ...
        onsessioninitialized: /* @__PURE__ */ __name(async (sid) => {
          ...
          server.server.oninitialized = async () => {
            ...
            const clientInfo = server.server.getClientVersion?.() || server.server.getClientInfo?.() || {};
            ...
            if (sessionData2) {
              ...
              sessionData2.clientInfo = clientInfo;
            }
...
function isOAuthProxy(oauth) {
  return oauth.type === "proxy";
}
__name(isOAuthProxy, "isOAuthProxy");
function createAuthorizeHandler(oauth) {
  return async (c) => {
...
__name(createAuthorizeHandler, "createAuthorizeHandler");
function createTokenHandler(oauth) {
  return async (c) => {
    try {
      ...
      const data = contentType.includes("application/x-www-form-urlencoded") ? Object.fromEntries(new URLSearchParams(rawBody)) : JSON.parse(rawBody);
      if (!response.ok) {
...
function setupOAuthRoutes(app, oauth, baseUrl) {
  ...
  const handleAuthorizationServerMetadata = /* @__PURE__ */ __name(async (c) => {
    ...
    if (proxyMode) {
      ...
      return c.json({
        ...
        token_endpoint_auth_methods_supported: proxy.clientSecret ? ["client_secret_post", "none"] : ["none"],
...
function createBearerAuthMiddleware(oauth, baseUrl) {
  return async (c, next) => {
    if (c.req.method === "HEAD") {
      return next();
    }
...
async function setupOAuthForServer(app, oauth, baseUrl, state) {
  if (state.complete) {
    return state;
  }
  const proxyMode = isOAuthProxy(oauth);
...
// src/server/middleware/mcp-middleware.ts
...
var MCPServerClass = class {
   ...
   * @internal
   */
  _registerMcpMiddleware(pattern, handler) {
   ...
   * ```
   */
  async proxy(config, options) {
    const { MCPClient: MCPClient2 } = await Promise.resolve().then(() => (init_client(), client_exports));
    const { mountSession: mountSession2 } = await Promise.resolve().then(() => (init_proxy_client(), proxy_client_exports));
    if (config && typeof config.callTool === "function") {
      await mountSession2(this, config, options?.namespace);
      return;
    }
    const proxyClient = new MCPClient2({
      mcpServers: config,
      onSampling: /* @__PURE__ */ __name(async (params) => {
        try {
          const { getRequestContext: getRequestContext2 } = await Promise.resolve().then(() => (init_context_storage(), context_storage_exports));
          const { findSessionContext: findSessionContext2 } = await Promise.resolve().then(() => (init_tool_execution_helpers(), tool_execution_helpers_exports));
    ...
    for (const [namespace, session] of Object.entries(sessions)) {
      await mountSession2(this, session, namespace);
    }
  }
  /**
  ...
  addWidgetTool(toolDefinition, toolCallback) {
    ...
    for (const [sessionId, session] of this.sessions) {
      ...
      const toolEntry = {
        ...
        inputSchema,
        outputSchema: registration.config.outputSchema,
  ...
  syncRegistrationsFrom(other) {
    ...
    const wrapHandler = /* @__PURE__ */ __name((rawHandler, session, sessionId) => {
      ...
      return async (params, extra) => {
        ...
        const resolvedSession = (sessionId ? sessions.get(sessionId) : void 0) ?? foundSession;
        const requestMeta = extra?._meta && Object.keys(extra._meta).some((k) => k !== "progressToken") ? Object.fromEntries(
        ...
        const mwCtx = {
          method: "tools/call",
          params,
          session: sessionId ? { sessionId } : void 0,
          auth: requestContext?.get("auth"),
          state: /* @__PURE__ */ new Map()
        };
        const innerFn = /* @__PURE__ */ __name(async () => {
          if (mwCtx.auth && !enhancedContext.auth) {
            enhancedContext.auth = mwCtx.auth;
          }
        ...
        const executeCallback = /* @__PURE__ */ __name(() => composeMiddleware(this.mcpMiddlewares, "tools/call", innerFn)(mwCtx), "executeCallback");
        if (requestContext) {
          return await runWithContext(requestContext, executeCallback);
        }
        return await executeCallback();
    ...
    const createToolEntry = /* @__PURE__ */ __name((name, config, handler, nativeServer, session, sessionId) => {
      let inputSchema;
      if (config.schema) {
      ...
      const wrappedHandler = session?.server ? wrapHandler(handler, session, sessionId) : handler;
      return {
        title: config.title,
    ...
    const registerPromptOnSession = /* @__PURE__ */ __name((server, name, config, handler) => {
      ...
      const wrappedHandler = /* @__PURE__ */ __name(async (...args) => {
        ...
        const { convertToolResultToPromptResult: convertToolResultToPromptResult2 } = await Promise.resolve().then(() => (init_conversion(), conversion_exports));
    ...
    const registerResourceOnSession = /* @__PURE__ */ __name((server, name, config, handler) => {
      const wrappedHandler = /* @__PURE__ */ __name(async (extra) => {
        ...
        const { convertToolResultToResourceResult: convertToolResultToResourceResult2 } = await Promise.resolve().then(() => (init_conversion2(), conversion_exports2));
  ...
  getServerForSession(sessionId) {
    ...
    for (const [name, registration] of this.registrations.tools) {
      ...
      const wrappedHandler = /* @__PURE__ */ __name(async (params, extra) => {
        ...
        const session = (sessionId ? this.sessions.get(sessionId) : void 0) ?? foundSession;
      ...
      const wrappedHandler = /* @__PURE__ */ __name(async (params, extra) => {
        ...
        const innerFn = /* @__PURE__ */ __name(async () => {
          ...
          const result = await handler(
            mwCtx.params,
            handler.length >= 2 ? enhancedCtx : void 0
          );
          if ("messages" in result && Array.isArray(result.messages)) {
            return result;
          }
    ...
    for (const [_key, registration] of this.registrations.resources) {
      ...
      const wrappedHandler = /* @__PURE__ */ __name(async (extra) => {
        ...
        const innerFn = /* @__PURE__ */ __name(async () => {
          const { enhancedCtx } = buildHandlerContext(sessionId, this.sessions);
          const result = await handler(
            handler.length >= 1 ? enhancedCtx : void 0
          );
          if ("contents" in result && Array.isArray(result.contents)) {
    ...
    newServer.server.setRequestHandler(
      import_zod5.z.object({ method: import_zod5.z.literal("logging/setLevel") }).passthrough(),
      (async (request, extra) => {
        const level = request.params?.level;
        if (!level) {
          throw new import_types3.McpError(
            import_types3.ErrorCode.InvalidParams,
        ...
        throw new import_types3.McpError(import_types3.ErrorCode.InternalError, "Could not find session");
      })
    );
   ...
   * @internal
   */
  _wrapListHandlers(nativeSrv, sessionId) {
    const handlers = nativeSrv.server?._requestHandlers;
    if (!handlers) return;
    const wrapListMethod = /* @__PURE__ */ __name((method, resultKey) => {
      const original = handlers.get(method);
      if (!original) return;
      if (original.__mcpListWrapped) return;
      const mcpMiddlewares = /* @__PURE__ */ __name(() => this.mcpMiddlewares, "mcpMiddlewares");
      const wrapped = /* @__PURE__ */ __name(async (req, extra) => {
        const mwCtx = {
          method,
          params: {},
          session: sessionId ? { sessionId } : void 0,
  ...
  async listen(port) {
    ...
    await mountWidgets(this, {
      ...
      ...process.env.MCP_USE_WIDGETS_DIR ? { resourcesDir: process.env.MCP_USE_WIDGETS_DIR } : {}
    });
    await this.mountMcp();
    ...
    if (process.env.NODE_ENV !== "production") {
      try {
        const { generateToolRegistryTypes: generateToolRegistryTypes2 } = await Promise.resolve().then(() => (init_tool_registry_generator(), tool_registry_generator_exports));
  ...
  async close() {
    if (this._httpServerClose) {
      ...
      this._httpServerClose = void 0;
      this._httpServerForceClose = void 0;
      await close();
    }
  }
  /**
...
// src/server/oauth/providers/clerk.ts
var import_jose3 = require("jose");
var ClerkOAuthProvider = class {
  static {
    __name(this, "ClerkOAuthProvider");
  }
  config;
  issuer;
  jwks = null;
  ...
  async verifyToken(token) {
    ...
    try {
      ...
      const result = await (0, import_jose3.jwtVerify)(token, this.getJWKS(), verifyOptions);
      return result;
    } catch (error2) {
      throw new Error(`Clerk JWT verification failed: ${error2}`);
    }
  }
  getUserInfo(payload) {
  ...
  getScopesSupported() {
    return this.config.scopesSupported ?? ["profile", "email", "offline_access"];
  }
  getGrantTypesSupported() {
    return ["authorization_code", "refresh_token"];
  }
...
var KeycloakOAuthProvider = class {
  ...
  getJWKS() {
    if (!this.jwks) {
      this.jwks = (0, import_jose4.createRemoteJWKSet)(
...
var WorkOSOAuthProvider = class {
  ...
  getJWKS() {
    if (!this.jwks) {
      this.jwks = (0, import_jose5.createRemoteJWKSet)(new URL(`${this.issuer}/oauth2/jwks`));
    }
    return this.jwks;
  }
  async verifyToken(token) {
    if (this.config.verifyJwt === false) {
      ...
      const payload = (0, import_jose5.decodeJwt)(token);
      return { payload };
    }
    try {
      const result = await (0, import_jose5.jwtVerify)(token, this.getJWKS(), {
        issuer: this.issuer
      });
      return result;
    } catch (error2) {
...
// src/server/oauth/providers/better-auth.ts
var import_jose6 = require("jose");
var BetterAuthOAuthProvider = class {
  static {
    __name(this, "BetterAuthOAuthProvider");
  }
  config;
  authURL;
  issuer;
  ...
  constructor(config) {
    this.config = config;
    this.authURL = config.authURL.endsWith("/") ? config.authURL.slice(0, -1) : config.authURL;
    this.issuer = this.authURL;
    this.jwksUrl = `${this.authURL}/jwks`;
  }
  getJWKS() {
    if (!this.jwks) {
      this.jwks = (0, import_jose6.createRemoteJWKSet)(new URL(this.jwksUrl));
    }
    return this.jwks;
  }
  async verifyToken(token) {
    if (this.config.verifyJwt === false) {
      ...
      const payload = (0, import_jose6.decodeJwt)(token);
      return { payload };
    }
    try {
      const result = await (0, import_jose6.jwtVerify)(token, this.getJWKS(), {
        issuer: this.issuer
      });
      return result;
    } catch (error2) {
      throw new Error(`Better Auth JWT verification failed: ${error2}`);
    }
  }
  getUserInfo(payload) {
...
function oauthClerkProvider(config = {}) {
  const frontendApiUrl = config.frontendApiUrl ?? getEnv("MCP_USE_OAUTH_CLERK_FRONTEND_API_URL");
  if (!frontendApiUrl) {
    throw new Error(
      "Clerk frontendApiUrl is required. Set MCP_USE_OAUTH_CLERK_FRONTEND_API_URL environment variable or pass frontendApiUrl in config."
    );
  }
...
__name(oauthClerkProvider, "oauthClerkProvider");
function oauthBetterAuthProvider(config = {}) {
  const authURL = config.authURL ?? getEnv("MCP_USE_OAUTH_BETTER_AUTH_URL");
  if (!authURL) {
    throw new Error(
      "Better Auth authURL is required. Set MCP_USE_OAUTH_BETTER_AUTH_URL environment variable or pass authURL in config."
    );
  }
...
__name(oauthBetterAuthProvider, "oauthBetterAuthProvider");
...
// src/server/oauth/oauth-proxy.ts
var import_jose7 = require("jose");
function jwksVerifier(config) {
  if (!config.jwksUrl) {
  ...
  const jwks = (0, import_jose7.createRemoteJWKSet)(new URL(config.jwksUrl));
  return async (token) => {
    try {
      const result = await (0, import_jose7.jwtVerify)(token, jwks, {
        issuer: config.issuer,
        ...config.audience ? { audience: config.audience } : {}
      });
      return { payload: result.payload };
    } catch (error2) {
      throw new Error(`JWKS verification failed: ${error2}`);
    }
  };
}
__name(jwksVerifier, "jwksVerifier");
function defaultGetUserInfo(payload) {
  const scope = payload.scope;
  return {
    userId: payload.sub,
    email: payload.email,
...
__name(defaultGetUserInfo, "defaultGetUserInfo");
function oauthProxy(config) {
  ...
  return {
    ...
    getIssuer: /* @__PURE__ */ __name(() => config.issuer, "getIssuer"),
    getAuthEndpoint: /* @__PURE__ */ __name(() => config.authEndpoint, "getAuthEndpoint"),
    getTokenEndpoint: /* @__PURE__ */ __name(() => config.tokenEndpoint, "getTokenEndpoint"),
    getScopesSupported: /* @__PURE__ */ __name(() => scopes, "getScopesSupported"),
    getGrantTypesSupported: /* @__PURE__ */ __name(() => grantTypes, "getGrantTypesSupported"),
...
// src/server/widgets/adapters/index.ts
init_mcp_apps();
init_apps_sdk();
...
var import_cors3 = require("hono/cors");
...
function mountMcpProxy(app, options = {}) {
  ...
  if (enableLogging) {
    app.use(`${basePath}/*`, (0, import_logger.logger)());
  }
...
var import_cors4 = require("hono/cors");
  ...
  app.get(`${basePath}/metadata`, async (c) => {
    try {
      ...
      if (!response || !response.ok) {
        ...
        const fallbackUrls = [metadataUrl.toString()];
        const mcpServerUrl = c.req.query("mcp_url");
        if (mcpServerUrl) {
          try {
            const parsedMcpUrl = new URL(mcpServerUrl);
            const requestHost = new URL(c.req.url).host;
            const metadataLooksLocal = metadataUrl.host === requestHost || metadataUrl.host === "localhost:3000" || metadataUrl.pathname.includes("/inspector/api/");
